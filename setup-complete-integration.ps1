...
# Step 4: Install Node.js for ClickUp MCP server (if needed)
...
# Step 5: Setup ClickUp MCP Server
Write-Step "Setting up ClickUp MCP Server..."

$clickupPath = Join-Path $WorkspacePath "integrations\clickup"
if (-not (Test-Path $clickupPath)) {
    New-Item -ItemType Directory -Path $clickupPath -Force | Out-Null
}

Set-Location $clickupPath

# Install ClickUp dependencies
if (-not (Test-Path "package.json")) {
    npm init -y
    npm install axios dotenv
    Write-Success "ClickUp MCP dependencies installed"
}

# Step 6: Create VS Code Workspace Configuration
...
# Step 7: Install Auto-Save Service
...
if (Test-Path $GitKrakenPath) {
    try {
        ...
        Write-Warning "GitKraken started but may need manual workspace selection"
    }
} else {
